...
using UnityEngine.XR.ARKit;
    ...
    {
        public static ARWorldMapController Instance;
        public bool beaconShow;
        public GameObject UI;
        public LayerMask showDebugLayerMask;
        public LayerMask cullDebugLayerMask;
        private GameObject ARPlanePrefab;
        public bool showDebug = true;
        public bool loadingInProgress = false;
        public ARPlaneManager arPlaneManager;
        public TextAsset worldMapAsset;
        public Image debugButtonToggle;
        ...
        IEnumerator Load()
        {
            MasterManager.Instance.waitingForWorldMap = true;
            var sessionSubsystem = (ARKitSessionSubsystem)m_ARSession.subsystem;
            if (sessionSubsystem == null)
            {
            ...
            allBytes.AddRange(worldMapAsset.bytes);
            ...
            Log("Applied! ARWorldMap to current session.");
            MasterManager.Instance.waitingForWorldMap = false;
            MasterManager.Instance.worldMapLoaded = true;
        }
    ...
    void SaveAndDisposeWorldMap(ARWorldMap worldMap)
        ...
        public void ToogleViewDebug()
        {
            SetShowTrackables(!showDebug);
            /*var planes = FindObjectsOfType<ARPlaneMeshVisualizer>();
            foreach (var plane in planes)
            {
                var renderer = plane.GetComponent<MeshRenderer>();
                if (renderer != null)
                    renderer.enabled = showDebug;
            }*/
        }

        public void SetShowTrackables(bool b)
        {
            showDebug = b;
            //arPlaneManager.SetTrackablesActive(showDebug);
            if (!showDebug)
            {
             //   ARPlanePrefab = arPlaneManager.planePrefab;
             //   arPlaneManager.planePrefab = null;
                Camera.main.cullingMask = cullDebugLayerMask;
            }
            else
            {
             //   arPlaneManager.planePrefab = ARPlanePrefab;
                Camera.main.cullingMask = showDebugLayerMask;
            }
            //arPlaneManager.enabled = showDebug;
            debugButtonToggle.color = showDebug ? Color.green : Color.white;
        }
        ...
        public void BeaconShowToggle()
        {
            beaconShow = !beaconShow;
            var beacons = FindObjectsOfType<Beacon>();
            foreach (var b in beacons)
            {
                b.GetComponent<MeshRenderer>().enabled = beaconShow;
            }
        }
    }